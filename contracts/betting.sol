...
contract OddsCalculator is ReentrancyGuard, Ownable {
    ...
    uint256 public totalBets1;
    uint256 public totalBets2;
    uint256 public totalBets0;
    ...
    function getSmallestChoiceAndLastBetIndex()
        public
        view
        returns (
        ...
        for (uint256 i = 0; i <= lastBetIndex; i++) {
            if (allBets[i].choice == _choice) {
                uint256 contributionPercentage =
                    (allBets[i].amount * 100) / totalContributedAmount;
                contributionPercentages[allBets[i].bettor] = contributionPercentage;
                emit ContributionPercentage(
                    allBets[i].bettor,
        ...
        for (uint256 i = 0; i <= lastBetIndex; i++) {
            if (allBets[i].choice == _choice) {
                uint256 tokensToTransfer =
                    (totalTokensInContract *
                        contributionPercentages[allBets[i].bettor]) / 100;
    ...
    function getBetDetails(address _user, uint256 _index)
        public
        view
        returns (uint256 choice, uint256 amount)
    {
    ...
    function getAllBetDetails(uint256 _index)
        public
        view
        returns (uint256 choice, uint256 amount)
    {
